...
/* 单节锂电池电压的产品显示近似，只用于 UI 提示和低电量趋势，不作为容量计量
 * 或保护阈值，也不代表充电芯片已确认充满。
 * 按产品要求保留 3.40 V=0%、3.55 V=15%，将原 4.10 V=90% 调整为 100%；
 * 其间沿用原电压节点，把原 15～90% 区间伸展至 15～100%，节点百分比取最近的 5%。
 * 相邻点之间线性插值，4.10 V 以上封顶 100%。该表为显示约定，未经容量实测标定。
 * UI 侧 julia_avatar_set_battery_status() 会把结果取整到 5% 步进（≥98% 显示 100%）。 */
static const battery_curve_point_t s_curve[] = {
    {3400, 0}, {3550, 15}, {3650, 30}, {3750, 55},
    {3850, 70}, {3950, 90}, {4100, 100},
};
