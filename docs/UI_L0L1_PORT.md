...
运行期电量来自 Waveshare 板载 `BAT_ADC`：GPIO8/ADC1_CH7 采样值按200K/100K分压乘3，32次平均后做低通滤波，并按当前单节3.7V、750mAh锂电池的非线性表估算百分比。保留老师指定的3.40V=0%、3.55V=15%标定点，高端恢复为4.10V=90%、4.20V=100%，相邻点线性插值；UI按5%取整，避免暗示不存在的1%精度。电池模块只维护 NORMAL／LOW 提示状态，不进入 S0～S8 行为FSM。有效电池始终在状态文字上方一行显示电量，并与状态左对齐：电量位于 `(60,80)`，状态位于 `(60,100)`，离线提示位于 `(60,120)`。NORMAL显示黑色 `BAT xx%`；15%及以下进入LOW并显示红色 `LOW xx%`，恢复到25%才退出；尚未取得电量或电压无效时隐藏。
