...
眨眼在独立任务中执行，音频、RTC、SD 和 Wi-Fi 初始化同时进行；网络服务和行为呈现受启动门槛保护，不覆盖动画。动画和初始化均完成后，FSM 由 S0 直接进入等待唤醒词的 S3，再启动闲置呈现、夜间与运动输入，并放行 MQTT／WSS；S1 只表示对话结束后的免唤醒陪伴期。动画任务创建失败时回退顺序执行，OTA 启动检查顺序不变。
...
| S7.1 断联 | 使用 `main/ui/generated/doze_frame_preview.png` 生成的内嵌闭眼立绘，固定 50% 背光，三秒后进入 S3 |
