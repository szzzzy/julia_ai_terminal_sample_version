...
眨眼在显示分支中以受限背光执行，同时推进音频、本地资源与 Wi-Fi。显示资源与音频/FSM 等必要资源成功后，MQTT/WSS 握手可与动画尾段重叠。只有动画结束且本地关键能力成功，FSM 才能接管；连接已判决则进入 S3，否则固定开机亮度（当前 50%）显示 S0 CONNECTING，直到成功或默认 30 秒期限到达。FSM 资源准备不会调用呈现或启动行为计时。动画播放失败可降级为静态画面；显示能力失败走关键故障。

启动等待显示 `S0 CONNECTING`；判决后 S3 始终显示原有 `S3 STANDBY`，仅用现有红色 `offline` 标签区分联网状态，恢复后清除标签；不再用 READY、OFFLINE 或 CONNECTING 替换 S3 文字。首次 30 秒云连接超时不再借用 S7.1，后续运行期断线仍保留 S7.1 和原提示声音。没有新增行为状态或视觉体系。

日志使用单调时间记录 `animation_finished`、`local_ready_s0`、`connection_decision`、`local_handoff`、`cloud_gate_open`、`cloud_business_ready`；本地就绪与云可交互必须分开测量。旧阶段 250ms 和末段 1500ms 等待已移除，电池阶段采样合并为启动一次。参见 [启动说明](BOOT_INITIALIZATION.md)。
