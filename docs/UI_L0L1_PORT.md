...
运行期电量来自 Waveshare 板载 `BAT_ADC`：GPIO9/ADC1_CH8 采样值按200K/100K分压乘3，32次平均后再做低通滤波，并按单节锂电池电压曲线估算百分比。屏幕顶部显示 `BAT xx%`，15%及以下使用红色；不在2500～4350mV窗口时隐藏。USB供电且未接电池时，充电芯片仍可能让BAT节点出现电压，因此“present”只表示电压合理，不是物理插头检测。
