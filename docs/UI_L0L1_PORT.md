...
| 行为状态 | 呈现 |
| --- | --- |
| S0 开机、S1 陪伴、S5 静默、S7 故障、S8 OTA | 调试阶段共用 Companion 基础立绘，由左上状态码区分 |
...
`julia_motion.c` 每 100ms 读取 QMI8658；加速度三轴差值和门限提高到 0.80g、陀螺仪幅值门限提高到 120°/s，并要求连续 8 次（约 800ms）命中，触发后冷却 10 秒。当前只在 S6 检测运动并恢复显示，播放期间不触发；它不会把 S3/S5/S6 切换到 S4。
