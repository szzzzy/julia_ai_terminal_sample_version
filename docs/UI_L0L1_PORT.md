...
| `JULIA_BATTERY stage=... vbat=...` | 对应阶段的 GPIO9 校准电压快照及采样范围 |
...
运行期电量来自 Waveshare 板载 `BAT_ADC`：GPIO9/ADC1_CH8 采样值按200K/100K分压乘3，32次平均后再做低通滤波，并按单节锂电池电压曲线估算百分比。电池模块自身维护 NORMAL／LOW／CHARGING 提示状态，不进入 S0～S8 行为FSM。屏幕顶部正常显示 `BAT xx%`；15%及以下显示红色 `LOW xx%`，恢复到25%才退出；连续电压上升推测为充电后显示绿色 `CHG xx%`并覆盖低电量，拔电压降确认后若仍低电量则恢复 LOW。

ETA6098 的 STAT 未接入可读GPIO，CHARGING只能由电压趋势推测。USB供电且未接电池时，充电芯片也可能让BAT节点出现电压，因此“present”只表示电压合理，不是物理插头检测；充电提示不得作为计费、安全保护或精确充电完成判据。
...
默认 S3 背光呼吸范围为 5%–30%，周期 4000ms；S6 停止呼吸、将背光置为 0%，并让 ST77916 执行 `DISPOFF + SLPIN`。只有 FSM 离开 S6 才执行 `SLPOUT + DISPON`，面板稳定等待约 120ms；activity、断链、运动和夜间窗口结束都不能旁路点亮。这仍不等于芯片进入硬件深度睡眠。

### 墙钟调度

`julia_time.c` 从 PCF85063 恢复有效时间，在获得 IP 后进行 SNTP 同步并写回 RTC；默认时区 `CST-8`。

`julia_night_schedule.c` 每 5 秒检查有效墙钟：默认夜间为 23:00–07:00；22 点仍会投递现有 `EVT_BEDTIME`，但当前 FSM 不用它改变状态。满足夜间条件的 S1/S3/S5 先经过约 5 分钟宽限，活动对话会延后进入 S6。07:00 只结束调度所有权，不改变 S6 呈现；设备仍保持暗屏，直到检测到唤醒词。时间无效时不执行这些时间判定。

### 运动输入

`julia_motion.c` 在非 S6 时不读取 QMI8658；进入 S6 后每 200ms 采样，加速度三轴差值门限为 0.80g、陀螺仪幅值门限为 120°/s，并要求连续 4 次（约 800ms）命中，触发后仅记录诊断并冷却 10 秒。运动不再直接恢复显示；S6 只能由唤醒词事件离开。
