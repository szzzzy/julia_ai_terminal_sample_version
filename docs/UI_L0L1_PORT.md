...
眨眼在主启动任务中以受限背光顺序执行，完成后才启动音频、RTC 和 SD；各高电流阶段默认错开250ms。FSM 由 S0 直接进入等待唤醒词的 S3 后，才最后启动 Wi-Fi；CPU 上限在启动 Wi-Fi 之前已从 `CONFIG_JULIA_BOOT_CPU_MAX_FREQ_MHZ`（80MHz 启动档）切到 `CONFIG_JULIA_RUNTIME_CPU_MAX_FREQ_MHZ`（160MHz 运行档），Wi-Fi 启动后再经过 `CONFIG_JULIA_BOOT_SETTLE_DELAY_MS`（默认 1500ms）稳定期。S1 只表示对话结束后的免唤醒陪伴期，OTA 启动检查顺序不变。
...
运行期电量来自 Waveshare 板载 `BAT_ADC`：GPIO8/ADC1_CH7 采样值按200K/100K分压乘3，按 `CONFIG_JULIA_BATTERY_SAMPLE_COUNT`（当前 16）次平均后做低通滤波，并按当前单节3.7V、750mAh锂电池的非线性表估算百分比。保留老师指定的3.40V=0%、3.55V=15%标定点，高端恢复为4.10V=90%、4.20V=100%，相邻点线性插值；UI按5%取整，避免暗示不存在的1%精度。电池模块只维护 NORMAL／LOW 提示状态，不进入 S0～S8 行为FSM。有效电池始终在状态文字上方一行显示电量，并与状态左对齐：电量位于 `(60,80)`，状态位于 `(60,100)`，离线提示位于 `(60,120)`。NORMAL显示黑色 `BAT xx%`；15%及以下进入LOW并显示红色 `LOW xx%`，恢复到25%才退出；尚未取得电量或电压无效时隐藏。

ETA6098 的 STAT 未接入可读GPIO，因此软件不再显示绿色CHG或推测充电状态。用户直接观察板载红灯：接入系统电池时常亮表示充电、熄灭表示充满；未接系统电池时灯态不确定。USB供电且未接电池时，充电芯片也可能让BAT节点出现电压，因此“present”只表示电压合理，不是物理插头检测。
...
独立眼层的生成基准坐标为左眼 `(112,108)`、右眼 `(194,108)`。为覆盖闭眼时底图下缘残留，半闭帧临时下移 1px、全闭帧下移 3px；恢复睁眼时回到基准坐标。该偏移只影响周期眨眼和持续闭眼帧，不移动正常睁眼位置。
