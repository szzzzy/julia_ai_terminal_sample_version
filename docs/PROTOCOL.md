...
连接与 socket 读写仍由单一会话任务负责；播放经缓冲交给独立任务，文件每轮推进一个块。MIC 上行先进入 256 帧（约 5.12 秒）的 PSRAM ring，每轮最多发送 4 帧；控制队列同样每轮最多处理 4 条。单次 socket 写等待配置为 500ms。`WANT_READ`、`WANT_WRITE` 和 socket `EAGAIN` 保持同一写入地址及剩余长度重试，WebSocket 帧头与载荷共享 2 秒绝对期限；期限耗尽或永久错误才结束连接。SD 文件打开／读取和 TLS 自身仍可能耗时，不能将这些设置解释为整条命令链路的硬实时保证。TCP／TLS 在线不表示 ASR 或回答生成仍正常。
...
序号在 MIC 任务内递增，不在每次 `MIC_START` 或 WSS 重连时清零；考虑 uint32 回绕。PSRAM ring 满时仍可能产生序号缺口，没有音频重传。ring 绑定 WSS 连接 generation：同一连接内保留积压，连接结束即全部丢弃，新连接绝不重放旧 PCM。校验和只用于载荷一致性检查，不是密码学完整性机制。
