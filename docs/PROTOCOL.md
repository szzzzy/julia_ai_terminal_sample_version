...
- 独立服务状态启动为 `CONNECTING`；MQTT/WSS 门控开放前建立默认 30 秒期限，只有 MQTT、WSS 和语音会话同步全部就绪才 ONLINE。S0 期间完成会话同步；初始成功进入 S3（不显示 offline），初始超时进入 S3（显示 offline），后台继续重连。运行期首次从 `ONLINE` 变为 `OFFLINE` 时，S7.1 播放固件内嵌的 16kHz 单声道 PCM16 提示语音并显示 `S7.1 DISCONNECTED` 三秒，不记录严重故障也不复位。提示结束后，S3 返回原状态；S5/S6 主动休眠不进入断联提示；S1/S2/S4 均绑定旧 WSS generation，断联后进入 S3，即使提示期间已经重连也不恢复旧会话。保持 `OFFLINE` 期间的重复断联或另一链路随后断开只更新原因，不重复播报；MQTT/WSS 全部恢复后隐藏标签并允许下一轮提示。
...
| `SPKS <rate>` | 本地收音版本仅在 S2.2 接受正常回答并进入 S2.3；关闭本地收音的兼容版本另允许 S4 等待中的云端 WAKE_REPLY；其他状态拒绝 |
...
上图为关闭本地收音的兼容路径。该路径的服务器应等待匹配 `interaction_id` 的 `state_ready` 再发送唤醒回应；不要把裸 `SPKS` 当作任意状态下的对话启动命令。

当前 `CONFIG_JULIA_LOCAL_CAPTURE_ENABLE=y` 的固件在有效 `wake_detected` 同步提交 S4 后，立即播放内嵌的 `wake_prompt_16k_mono_16bit.wav`（16kHz、单声道、16bit，1.968 秒）。`state_ready` 仍由 WSS owner 按现有流程发送，表示 S4 已提交，不等待本地播放完成。云端不再合成或下发该轮唤醒应答，可与本地播放并行准备接收首句；固件不新增云端就绪消息或播放完成消息。

本地应答播放期间不采集首句。播放完成、现有约 600ms 防回声间隔结束且 S4 回执已发出后，按 capture-v1 恢复收音；不等待云端另行确认，不依赖 `MIC_START`。这要求云端在该时段内准备好接收，固件没有新增准备超时后的首句补传机制。重复唤醒不会重播；S4 不接受云端 `SPKS`，迟到的 PCM／`SPKE` 不影响本地唤醒播放。断线或退出 S4 取消本轮播放，播放启动或输出失败走会话恢复。普通 S2 回答仍走原有网络播放路径。
