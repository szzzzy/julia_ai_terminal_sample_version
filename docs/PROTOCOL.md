...
连接与 socket 读写仍由单一会话任务负责；播放经缓冲交给独立任务，文件每轮推进一个块。每轮控制和 MIC 队列分别最多处理 4 条；单次 socket 写等待配置为 500ms。`WANT_READ`、`WANT_WRITE` 和 socket `EAGAIN` 保持同一写入地址及剩余长度重试，WebSocket 帧头与载荷共享 2 秒绝对期限；期限耗尽或永久错误才结束连接。SD 文件打开／读取和 TLS 自身仍可能耗时，不能将这些设置解释为整条命令链路的硬实时保证。TCP／TLS 在线不表示 ASR 或回答生成仍正常。
