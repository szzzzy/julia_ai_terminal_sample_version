...
| `CONFIG_JULIA_BAT_CONTROL_GPIO` | 电池供电保持脚；当前实机配置GPIO7，不同原理图／批次需按实机核对 |
| `CONFIG_JULIA_BOOT_CPU_MAX_FREQ_*`／`CONFIG_JULIA_RUNTIME_CPU_MAX_FREQ_*` | 启动与运行期 CPU 上限，当前默认80MHz／160MHz |
| `CONFIG_JULIA_LOCAL_INIT_TIMEOUT_MS` | 本地并行分支汇合期限，默认 20000ms；独立于云连接期限，超时不开放交互。旧 250ms/1500ms 固定等待已移除 |
| `CONFIG_JULIA_BOOT_BRIGHTNESS_PERCENT` | 开机动画背光上限，默认50% |
| `CONFIG_JULIA_BATTERY_DIAGNOSTICS`／`SAMPLE_COUNT` | GPIO8 BAT_ADC 启动阶段诊断、运行期监测及每次采样数 |
...
| `CONFIG_JULIA_SERVICE_INIT_TIMEOUT_SECONDS` | MQTT/WSS 门控开放前建立的首次业务连接期限，默认 30 秒；要求 MQTT、WSS 与会话同步全部就绪，S0 固定亮度等待判决，成功或超时后才进入 S3，超时显示 OFFLINE 并继续重连 |
